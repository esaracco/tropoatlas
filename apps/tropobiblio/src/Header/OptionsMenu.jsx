...
const OptionsMenu = () => {
  ...
  const isUserChanged = Boolean(
    previousUser && inventaireUser && previousUser !== inventaireUser,
  )

  const onConfirmSync = async () => {
    setShowSyncModal(false)
    await syncCollection({ forceRefresh: isUserChanged || forceRefresh })
  }
                ...
                label={
                  <div>
                    <div style={{ fontWeight: 600 }}>
                      {t("Force full re-synchronization of all books")}
                    </div>
                    <div
                      style={{
                    ...
                    >
                      {t(
                        "Otherwise, only added or removed books will be processed",
                      )}
                    </div>
                  </div>
                }
