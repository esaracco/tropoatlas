...
- **Instance Configuration via Environment**: Instance-wide configuration (data provider options, hardware LED colors, currencies, mapping fields) is strictly governed by environment variables (`.env` / Docker). Applications and plugins consume these variables directly without runtime local storage overriding.
