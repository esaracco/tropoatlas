...
- **Environment Variables Minimal Surface**: Only parameters that strictly require end-user customization (such as authentication tokens, usernames, instance identifiers, or LED hardware endpoints) MUST be exposed as environment variables (`.env` / Docker). Internal protocol mechanics, standard field identifiers, and provider limits must remain defined as codebase constants.
- **Instance Configuration via Environment**: Instance-wide configuration (credentials, hardware options, currencies) is strictly governed by environment variables (`.env` / Docker). Applications and plugins consume these variables directly without runtime local storage overriding.
- **Cache Preservation**: User preference stores (such as UI preferences and themes) MUST be preserved when clearing caches.
...
- **Offcanvas and Modals (Mobile UI)**: Stacking Bootstrap Modals over an open Offcanvas menu can cause backdrop conflicts when the modal is closed. 
  - If a button opens a global root-level Modal, the Offcanvas MUST be explicitly closed before opening the modal.
